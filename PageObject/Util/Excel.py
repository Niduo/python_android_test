...
class parseExcel(object):
    def __init__(self,excelPath):
        ...
        self.colorDict = {"red":'FFF3030',"green":'FF008B00'}
    # 设置当前要操作的sheet对象，使用index来获取相应的sheet
    def set_sheet_by_index(self,sheet_index):
        sheet_name = self.workbook.sheetnames[sheet_index]
        # sheet_name = self.workbook.get_sheet_names()[sheet_index]#get_sheet_names已弃用
        # self.sheet = self.workbook.get_sheet_by_name(sheet_name)   #get_sheet_names已弃用
    ...
    #获取当前要操作的sheet对象，使用sheet名称来获取相应的sheet
    ...
    #从默认sheet中获取某一列，第一列从零开始
    def get_single_col(self,col_no):
        return self.get_all_cols()[col_no]

    #从默认sheet中获取某一行，第一行从零开始
    def get_single_row(self,row_no):
        return  self.get_all_rows()[row_no]

    #从默认sheet中，通过行号和列号获取指定的单元格，注意行号和列号从一开始
    def get_cell(self,row_no,col_no):
        return self.sheet.cell(row=row_no,column=col_no)

    #获取默认sheet中，通过行号和列号获取指定单元格中的内容，注意行号和列号从一开始
    def get_cell_content(self,row_no,col_no):
        return self.sheet.cell(row=row_no,column=col_no).value

    #从默认sheet中，通过行号和列号想指定的单元格中写入指定内容，注意行号和列号从1开始
    ...
    #从默认sheet中，通过行号和列号向指定单元格中写入当前日期，注意行号和列号从一开始
    #调用此方法的时候，excel不要处于打开状态
...
if __name__=="__main__":
    p = parseExcel("C:\\Users\\Hunter\\Desktop\\hoomtest.xlsx")
    ...
    # print(u"获取某一列（2）对象：",p.get_single_col(2))
    # print(u"获取某一行对象（2）：",p.get_all_rows())
    # print(u"获取行号和列号（2，2）单元格：",p.get_cell(2,2))
    # print(u"取得行号和列号单元格的内容（2,2）",p.get_cell_content(2,2))
    # print(u"行号和列号写入内容（11，11）,：'yangyue'",p.write_cell_content(11,11,'yangyue'))
    # print(u"行号和列号写入内容：（13,13）",p.write_cell_current_time(13,13))